-- Úmit Platform Database Schema (Safe version)
-- Безопасная установка - пропускает уже существующие типы

CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- ============================================
-- CUSTOM TYPES (ENUMS) - безопасное создание
-- ============================================
...
DROP POLICY IF EXISTS "Needed items are viewable by everyone" ON public.needed_items;
DROP POLICY IF EXISTS "Request owners can manage needed items" ON public.needed_items;
DROP POLICY IF EXISTS "Donor offers are viewable by everyone" ON public.donor_offers;
DROP POLICY IF EXISTS "Authenticated users can create donor offers" ON public.donor_offers;
DROP POLICY IF EXISTS "Donors can update own offers" ON public.donor_offers;
DROP POLICY IF EXISTS "Donors can delete own offers" ON public.donor_offers;
DROP POLICY IF EXISTS "Responses are viewable by everyone" ON public.responses;
DROP POLICY IF EXISTS "Authenticated users can create responses" ON public.responses;
DROP POLICY IF EXISTS "Donors can update own responses" ON public.responses;
DROP POLICY IF EXISTS "Donors can delete own responses" ON public.responses;
DROP POLICY IF EXISTS "Offered items are viewable by everyone" ON public.offered_items;
DROP POLICY IF EXISTS "Response owners can manage offered items" ON public.offered_items;
DROP POLICY IF EXISTS "Shelters are viewable by everyone" ON public.shelters;
DROP POLICY IF EXISTS "Authenticated users can create shelters" ON public.shelters;
DROP POLICY IF EXISTS "Managers can update own shelters" ON public.shelters;
DROP POLICY IF EXISTS "Managers can delete own shelters" ON public.shelters;
DROP POLICY IF EXISTS "Shelter needs are viewable by everyone" ON public.shelter_needs;
DROP POLICY IF EXISTS "Shelter managers can manage needs" ON public.shelter_needs;
DROP POLICY IF EXISTS "Achievements are viewable by everyone" ON public.achievements;
DROP POLICY IF EXISTS "User stats are viewable by everyone" ON public.user_stats;
DROP POLICY IF EXISTS "Emergencies are viewable by everyone" ON public.emergencies;
DROP POLICY IF EXISTS "Request volunteers are viewable by everyone" ON public.request_volunteers;
DROP POLICY IF EXISTS "Volunteers can join requests" ON public.request_volunteers;
DROP POLICY IF EXISTS "Volunteers can leave requests" ON public.request_volunteers;
...
-- Готово!
SELECT 'Úmit Platform database schema installed successfully!' AS message,
